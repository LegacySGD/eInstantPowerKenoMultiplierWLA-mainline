			...
			<lxslt:component prefix="my-ext" functions="formatJson,retrievePrizeTable,getType">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNamesDesc)
					{
						...
						var matchedText = scenario.split('|')[1];
						var matchedNum  = matchedText.split('m')[0];
						var matchedAcc  = matchedText.split('m')[1];
						var prizeNames  = (prizeNamesDesc.substring(1)).split(',');
						var convertedPrizeValues = (prizeValues.substring(1)).split('|');

						const accs = [0,1,2,3,4,5,6];
						const multis = [1,1,2,3,4,5,10];
						...
						r.push('</td>');
						r.push('<td>');
						r.push(getTranslationByName("basePrize", translations));
						r.push('</td>');
						r.push('<td>');
						r.push(getTranslationByName("accumulators", translations));
						r.push('</td>');
						r.push('<td>');
						...
						r.push('</td>');
						r.push('<td class="tablebody">');
						r.push(convertedPrizeValues[getPrizeNameIndex(prizeNames,'m'+matchedNum)]);
						r.push('</td>');
						r.push('<td class="tablebody">');
						r.push(matchedAcc);
						r.push('</td>');
						r.push('<td class="tablebody">');
						r.push(multis[accs.indexOf(parseInt(matchedAcc))] + 'x');
						r.push('</td>');
						r.push('</tr>');
